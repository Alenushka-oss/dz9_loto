...
class Bag: #мешочек с бочонками
    ...
    def next(self):
        ...
        raise Exception('Мешочек пуст')

    def stats(self):
        print(f'В мешке {len(self.nums)} бочонков.')
...
class User(Computer):
    ...
    def step(self, num):
        ...
        while answer not in self.answers:
            answer = input('Зачеркнуть ли цифру? (y/n) ')
        if answer == 'y':
            if num in self.card.card_nums:
                self.card.modify(num)
                self.nums.remove(num)
...
class Game:
    ...
    def run(self, num_compics, num_users):
        ...
        while True:
            ...
            print(f'Из мешка вынут бочонок номер {num}!')
    ...
    def step(self, num):
        for player in self.players.values():
            ...
            if isinstance(player, User):
                if player.is_looser:
                    print(f'\nСОЖАЛЕЮ, {player.name}, ВЫ ПРОИГРАЛИ(((')
                    self.num_users -= 1
